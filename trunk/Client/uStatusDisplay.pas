...
begin
  ...
  begin
    ...
    sTime   := FormatDateTime('报名时间：yyyy-mm-dd',ClientDataSet1.FieldByName('开始时间').AsDateTime)+'~'+
               FormatDateTime('yyyy-mm-dd',ClientDataSet1.FieldByName('结束时间').AsDateTime);

    if IsStart then
      sState := '服务状态：网上报名服务已开启'
    else
      sState := '服务状态：网上报名服务已关闭';
  end;

  lsbRight.Items.Clear;
  imgIndexList.Clear;

  lsbRight.Items.Add(sfmc+#13+sTime+#13+sState);
  if IsStart then
    imgIndexList.Add('0')
  else
    imgIndexList.Add('1');
