...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, Buttons, Rio, ShellAPI, uZsbBmIntf,StrUtils,
...
begin
  ...
  begin
    ...
    if not DirectoryExists(sWebDir) then
      CreateDir(sWebDir);
    CreateVirtualDir(sVirDir,sWebDir,'专升本报名系统根目录',iWebSiteIndex+1,False);
  ...
  begin
    ...
    if not DirectoryExists(sWebDir) then
      CreateDir(sWebDir);
    CreateVirtualDir(sVirDir+'/Web',sWebDir,'专升本报名WEB子系统',iWebSiteIndex+1,False);
  end;
