...
uses InvokeRegistry, SysUtils, Types, uZsbBmIntf, DB, ADODB, DBClient, WebBrokerSOAP;
...
type
  ...
  public
    ...
    function GetBmStatus(const UserName:string):string;stdcall;
    ...
    function getbackPassword(const sfzh,xm,email:string):Boolean;stdcall;
    function SendTestMail(const email,subject,content:string):Boolean;stdcall;
  end;
...
function TZsbBm.getbackPassword(const sfzh, xm, email: string): Boolean;
var
  dm:TZsbBmSoapDM;
begin
  dm := TZsbBmSoapDM.Create(nil);
  try
    Result := dm.getbackPassword(sfzh,xm,email);
  finally
    dm.Free;
  end;
end;
...
function TZsbBm.GetBmStatus(const UserName: string): string;
var
  dm:TZsbBmSoapDM;
  sqlStr:string;
begin
  Result := '';
  dm := TZsbBmSoapDM.Create(nil);
  sqlStr := 'select 是否缴费 from view_考生报名表 where 登录名='+quotedstr(UserName);
  try
    dm.cds_Query.Close;
    dm.cds_Query.CommandText := sqlStr;
    dm.cds_Query.Open;
    if dm.cds_Query.Fields[0].AsBoolean then
      Result := '成功'
    else
    begin
...
function TZsbBm.SendTestMail(const email, subject, content: string): Boolean;
var
  dm:TZsbBmSoapDM;
begin
  dm := TZsbBmSoapDM.Create(nil);
  try
    Result := dm.SendEmail(email,subject,content);
  finally
    dm.Free;
  end;
end;
